...
## 🧱 Binary Heap Refresher (Quick)

* A **binary heap** is a complete binary tree (always filled left to right).
...
1. **Place the new value at the next available position** in the array.

   * This ensures the tree remains **complete**.
   * It goes on the **bottom row, far left as possible**.

2. **Bubbling Up** (a.k.a. "sift up", "heapify up"):

   * Compare the new value with its **parent**.
   * If it's **greater than the parent**, **swap them**.
   * Repeat until:

     * It's smaller than its parent, **or**
     * It becomes the **root**.

This restores the **heap property** (biggest always at the top).
...
Initial heap: `[20, 15, 10, 7, 8]`

You insert `16`.

Step 1: Place at the end → `[20, 15, 10, 7, 8, 16]`
...
* **Bubble up** by comparing to the parent (`(i-1) // 2` index).
* Stops bubbling when heap property is satisfied.
