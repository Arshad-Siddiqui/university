...
When data is sorted and indexableyou can cut the search pool in half each step making it way more efficient than going through every single element and checking it off the list in the worst case. We're looking at O(log n) instead of O(n).
